...
const Category_type = ({ data, type }) => {
    const navigate= useNavigate()
    
        const handle_navigate = (item,image_category) => {
            navigate('/share_category', { state: { item, type, image_category } });
        }
  
  return (
    <div className=" mt-20 relative p-20 bg-gray-100 rounded-lg shadow-md">
      ...
      <div className="mt-16 grid grid-cols-1 gap-6">
        ...
        {data.map((item, index) => (
          <div
            ...
            onClick={()=>handle_navigate(item.title,item.image)}
          >
            {/* Image */}
            <img
              src={item.image}
